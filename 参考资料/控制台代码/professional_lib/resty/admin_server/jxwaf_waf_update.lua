...
local function update_waf_conf_update_time(waf_node_uuid,user_name)
    ...
    if tonumber(count_sql_result[1].count) == 1 then
        local update_sql = "UPDATE jxwaf_node_monitor  SET  waf_conf_update_time = ? WHERE  user_name = ? and node_uuid = ?  ;"
        local update_sql_params = {now_time,user_name,waf_node_uuid}
...
function _M.waf_update()
  ...
  local waf_conf_md5 = waf_update_conf_data:get(user_name .. ":waf_conf_md5")
  ...
  local waf_conf_data = waf_update_conf_data:get(user_name .. ":waf_conf_data")
  if waf_conf_data and waf_conf_md5 then
    ...
    update_waf_conf_update_time(waf_node_uuid,user_name)
    ngx.status = 200
    ngx.header.content_type = "application/json"
    ngx.say(cjson.encode(return_data))
    return ngx.exit(200)
