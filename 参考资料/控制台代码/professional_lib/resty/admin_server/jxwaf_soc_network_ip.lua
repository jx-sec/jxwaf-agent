...
function _M.network_block()
  local check_param = {"waf_auth", "ip", "expire_time", "operator_type"}
	...
	local query_account_sql = [[SELECT user_name FROM `jxwaf_admin_account` where `waf_auth` = ?;]]
	local query_account_result = db_query.query_mysql(query_account_sql, {waf_auth})
