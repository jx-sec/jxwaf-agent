...
function _M.account_init_check()
  local return_data = {}
  return_data['open_regist'] = init_config['open_regist']
  return_data['result'] = true
  if init_config['open_regist'] == "true" then
    return_data['message'] = "open_regist"
...
function _M.get_otp_qr_url()
      if init_config['open_regist'] == "false" then
        local account_count_sql = 'SELECT COUNT(*) AS count FROM jxwaf_admin_account;'
        local account_count_sql_result = db_query.query_mysql(account_count_sql)
        if not account_count_sql_result then
    ...
    local tmp_data = ngx.shared.tmp_data
    tmp_data:set(session_uuid,otp_secret_key,86400)
...
function _M.account_regist()
  if init_config['open_regist'] == "false" then
     ...
     if otp_auth == 'true' then
      ...
      otp_secret_key = tmp_data:get(account_regist_session)
      if not otp_secret_key then
          response.fail_response("otp not init fail,session is null")
      end
      local otp_init = otp.totp_init(otp_secret_key)
      if not otp_init:verify_token(otp_auth_code) then
          response.fail_response("otp fail")
      end
      tmp_data:delete(account_regist_session)
    end
    local query_account_sql = [[SELECT * FROM `jxwaf_admin_account` where `user_name` = ? ;]]
    local query_account_sql_params = {user_name}
    local query_account_result,query_account_err = db_query.query_mysql(query_account_sql,query_account_sql_params)
    if not query_account_result then
        response.fail_response("query_account_sql exec  err:",query_account_err)
    end
    if #query_account_result ~= 0 then
        response.fail_response("account_name_has_been_registered,count is :",#query_account_result)
    end
    local user_password_md5 = tools.get_md5(user_password)
    local waf_auth_uuid = uuid.generate_random()
    local create_account_sql = [[INSERT INTO `jxwaf_admin_account` (`user_name`, `user_password`,`otp_auth`,`otp_secret_key`,`waf_auth`) VALUES ( ?,?,?,?,? );]]
    local create_account_sql_params = {user_name,user_password_md5,otp_auth,otp_secret_key,waf_auth_uuid}
...
function _M.get_waf_auth()
  local user_name = login_check.get_session()
  local query_account_sql = [[SELECT * FROM `jxwaf_admin_account` where `user_name` = ? ;]]
  local query_account_sql_params = {user_name}
  local query_account_result,err = db_query.query_mysql(query_account_sql,query_account_sql_params)
  if (not query_account_result) or (query_account_result and #query_account_result == 0) then
    response.fail_response("user_name get fail")
  end
  local waf_auth = query_account_result[1]['waf_auth']
  if waf_auth then
    response.success_response(waf_auth)
  else
    response.fail_response("user_name is null")
  end
end

function _M.edit_waf_auth()
  local user_name = login_check.get_session()
  local check_param = {"waf_auth"}
  local body_data = request_data.get_body_data(check_param)
  local waf_auth = body_data['waf_auth']
  local sql = "UPDATE jxwaf_admin_account SET  waf_auth = ? WHERE user_name = ?;"
  local sql_params = {waf_auth,user_name}
