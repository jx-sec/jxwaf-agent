...
local DEFAULT_PLANS_CONFIG = {
  daily_observe = {
    ...
    domain_access_limit_threshold = "500",
  ...
  attack_protect = {
    ip_access_limit_status = "true",
    ip_access_limit_stat_time = "30",
    ip_access_limit_threshold = "50",
    ip_access_limit_action = "block",
    ip_access_limit_action_extra_parameter = "auto",
    ip_access_limit_duration = "600",
    ip_count_limit_status = "true",
    ip_count_limit_stat_time = "30",
    ip_count_limit_threshold = "100",
    ip_count_limit_action = "block",
    ip_count_limit_action_extra_parameter = "auto",
    domain_access_limit_status = "true",
    domain_access_limit_stat_time = "30",
    domain_access_limit_threshold = "300",
    domain_access_limit_action = "block",
    domain_access_limit_action_extra_parameter = "auto",
    ssl_fingerprint_protection_status = "true",
    ssl_fingerprint_protection_action = "block",
    ssl_fingerprint_protection_action_extra_parameter = "auto",
    emergency_protection_status = "false",
    emergency_protection_action = "block",
    emergency_protection_action_extra_parameter = "auto"
  ...
  emergency_protect = {
    ip_access_limit_status = "true",
    ip_access_limit_stat_time = "10",
    ip_access_limit_threshold = "20",
    ip_access_limit_action = "block",
    ip_access_limit_action_extra_parameter = "auto",
    ip_access_limit_duration = "900",
    ip_count_limit_status = "true",
    ip_count_limit_stat_time = "10",
    ip_count_limit_threshold = "50",
    ip_count_limit_action = "block",
    ip_count_limit_action_extra_parameter = "auto",
    domain_access_limit_status = "true",
    domain_access_limit_stat_time = "10",
    domain_access_limit_threshold = "100",
    domain_access_limit_action = "block",
    domain_access_limit_action_extra_parameter = "auto",
    ssl_fingerprint_protection_status = "true",
    ssl_fingerprint_protection_action = "block",
...
local function create_default_flow_engine_protection(user_name, group_name)
  local plans_config_json = cjson.encode(DEFAULT_PLANS_CONFIG)
  local create_sql = [[INSERT INTO jxwaf_waf_group_flow_engine_protection 
    (user_name, group_name, engine_status, protection_plan, plans_config)
    VALUES (?,?,?,?,?);]]
  local create_sql_params = {user_name, group_name, "false", "daily_observe", plans_config_json}
...
function _M.get_group_flow_engine_protection()
  ...
  local target_plan = body_data['protection_plan']
  
  local count_sql = "SELECT COUNT(*) as count FROM jxwaf_waf_group_flow_engine_protection WHERE group_name = ? AND user_name = ?;"
  ...
  if tonumber(count_sql_result[1].count) == 0 then
    local success = create_default_flow_engine_protection(user_name, group_name)
    if not success then
  ...
  local sql = "SELECT * FROM jxwaf_waf_group_flow_engine_protection WHERE user_name = ? AND group_name = ?;"
  ...
  if query_result and #query_result > 0 then
    ...
    local current_plan = result.protection_plan or "daily_observe"
    local query_plan = target_plan or current_plan
    local plan_config = nil
    
    if result.plans_config and result.plans_config ~= ngx.null then
      local plans_config = cjson.decode(result.plans_config)
      if plans_config and plans_config[query_plan] then
        plan_config = plans_config[query_plan]
      end
    end
...
function _M.edit_group_flow_engine_protection()
  local user_name = login_check.get_session()
  local check_param = {"group_name", "protection_plan"}
  local body_data = request_data.get_body_data(check_param)
  local group_name = body_data['group_name']
  local protection_plan = body_data['protection_plan']
  
  local sql = "SELECT plans_config FROM jxwaf_waf_group_flow_engine_protection WHERE user_name = ? AND group_name = ?;"
  local sql_params = {user_name, group_name}
  local query_result, query_error = db_query.query_mysql(sql, sql_params)
  ...
  if query_result[1].plans_config and query_result[1].plans_config ~= ngx.null then
    plans_config = cjson.decode(query_result[1].plans_config) or {}
  end
  ...
  plans_config[protection_plan] = plan_config
  ...
  table.insert(update_fields, "protection_plan = ?")
  table.insert(update_params, protection_plan)
  
  table.insert(update_fields, "plans_config = ?")
  table.insert(update_params, cjson.encode(plans_config))
  
  table.insert(update_params, group_name)
  table.insert(update_params, user_name)
  
  local update_sql = "UPDATE jxwaf_waf_group_flow_engine_protection SET " .. table.concat(update_fields, ", ") .. " WHERE group_name = ? AND user_name = ?;"
