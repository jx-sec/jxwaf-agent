...
function _M.get_group_custom_upstream_address_list()
...
local count_sql = "SELECT COUNT(*) AS total FROM jxwaf_waf_group_custom_upstream_address WHERE `user_name` = ? AND `group_name` = ?  ;"
  ...
  local sql = "SELECT * FROM jxwaf_waf_group_custom_upstream_address  WHERE `user_name` = ? AND `group_name` = ? ORDER BY rule_order_time ASC LIMIT ? OFFSET ?;"
...
function _M.get_group_custom_upstream_address()
  local user_name = login_check.get_session()
  local check_param = {"group_name","rule_name"}
  local body_data = request_data.get_body_data(check_param)
  local group_name = body_data['group_name']
  local rule_name = body_data['rule_name']
  local sql = "SELECT * FROM jxwaf_waf_group_custom_upstream_address  WHERE `user_name` = ? AND `group_name` = ? AND `rule_name` = ?;"
...
function _M.create_group_custom_upstream_address()
  local user_name = login_check.get_session()
  local check_param = {"group_name","rule_name","rule_detail","rule_matchs","filter","source_ip","source_http_port","source_https_port"}
  local body_data = request_data.get_body_data(check_param)
  local group_name = body_data['group_name']
  local rule_name = body_data['rule_name']
  ...
  local count_sql = "SELECT COUNT(*) as count FROM jxwaf_waf_group_custom_upstream_address WHERE group_name = ? AND user_name = ?  AND rule_name = ?;"
  ...
  local create_sql = "INSERT INTO jxwaf_waf_group_custom_upstream_address (user_name,group_name,rule_name, rule_detail,rule_matchs,filter,source_ip,source_http_port,rule_order_time,source_https_port) VALUES (?,?,?,?,?,?,?,?,?,?);"
  local create_sql_params = {user_name,group_name,rule_name,rule_detail,rule_matchs,filter,source_ip,source_http_port,rule_order_time,source_https_port}
...
function _M.delete_group_custom_upstream_address()
  local user_name = login_check.get_session()
  local check_param = {"group_name","rule_name"}
  local body_data = request_data.get_body_data(check_param)
  local group_name = body_data['group_name']
  local rule_name = body_data['rule_name']
  local sql = "DELETE FROM jxwaf_waf_group_custom_upstream_address WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
...
function _M.edit_group_custom_upstream_address()
  local user_name = login_check.get_session()
  local check_param = {"group_name","rule_name","rule_detail","rule_matchs","filter","source_ip","source_http_port","source_https_port"}
  local body_data = request_data.get_body_data(check_param)
  local group_name = body_data['group_name']
  ...
  local source_http_port = body_data['source_http_port']
  local source_https_port = body_data['source_https_port']
  local sql = "UPDATE jxwaf_waf_group_custom_upstream_address  SET  rule_detail = ?,rule_matchs = ?,filter = ? ,source_ip = ? ,source_http_port = ?,source_https_port = ? WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
  local sql_params = {rule_detail,rule_matchs,filter,source_ip,source_http_port,source_https_port,group_name,user_name,rule_name}
  local sql_result,sql_error = db_query.query_mysql(sql,sql_params)
  if not sql_result then
    response.fail_response(sql_error)
  end
...
function _M.edit_group_custom_upstream_address_status()
  local user_name = login_check.get_session()
  ...
  local sql = "UPDATE jxwaf_waf_group_custom_upstream_address  SET  status = ? WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
...
function _M.exchange_group_custom_upstream_address_priority()
  local user_name = login_check.get_session()
  ...
  if exchange_type == "top" then
    local sql = "SELECT rule_order_time FROM jxwaf_waf_group_custom_upstream_address  WHERE `user_name` = ? AND `group_name` = ?  ORDER BY `rule_order_time` ASC limit 1;"
    ...
    local exchange_sql = "UPDATE jxwaf_waf_group_custom_upstream_address  SET  rule_order_time = ? WHERE group_name = ? AND user_name = ? AND rule_name = ? ;"
  ...
  elseif exchange_type == "exchange" then
    ...
    local rule_query_sql = "SELECT rule_order_time FROM jxwaf_waf_group_custom_upstream_address  WHERE `user_name` = ? AND `group_name` = ? AND `rule_name` = ? ;"
    ...
    local exchange_query_sql = "SELECT rule_order_time FROM jxwaf_waf_group_custom_upstream_address  WHERE `user_name` = ? AND `group_name` = ? AND `rule_name` = ?;"
    ...
    local rule_update_sql = "UPDATE jxwaf_waf_group_custom_upstream_address  SET  rule_order_time = ? WHERE group_name = ? AND user_name = ? AND rule_name = ?  ;"
    ...
    local exchange_update_sql = "UPDATE jxwaf_waf_group_custom_upstream_address  SET  rule_order_time = ? WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
...
function _M.backup_group_custom_upstream_address()
  local user_name = login_check.get_session()
  ...
  for _,rule_name in ipairs(rule_name_list) do 
    ...
    local rule_conf = {
      ...
      source_ip = rule_name_result['source_ip'],
      source_http_port = rule_name_result['source_http_port'],
      source_https_port = rule_name_result['source_https_port']
      }
    table.insert(rules,rule_conf)
  end
  ngx.status = 200
  ngx.header.content_type = 'application/json'
  ngx.header['Content-Disposition'] = 'attachment; filename="custom_upstream_address_data.json"'
  ngx.say(cjson.encode(rules))
  return ngx.exit(200)
end

function _M.load_group_custom_upstream_address()
  local user_name = login_check.get_session()
  ...
  for _,rule in ipairs(rules) do 
    ...
    local source_ip = rule['source_ip']
    local source_http_port = rule['source_http_port']
    local source_https_port = rule['source_https_port']
    local count_sql = "SELECT COUNT(*) as count FROM jxwaf_waf_group_custom_upstream_address WHERE group_name = ? AND user_name = ? AND rule_name = ? ;"
    local count_sql_params = {group_name,user_name,rule_name}
    local count_sql_result,count_error = db_query.query_mysql(count_sql,count_sql_params)
    if not count_sql_result then
    ...
    if tonumber(count_sql_result[1].count) == 0 then
      local rule_order_time = math.floor(ngx.now())
      local create_sql = "INSERT INTO jxwaf_waf_group_custom_upstream_address (user_name,group_name,rule_name,rule_detail,rule_matchs,filter,source_ip,source_http_port,source_https_port,rule_order_time) VALUES (?,?,?,?,?,?,?,?,?,?);"
      local create_sql_params = {user_name,group_name,rule_name,rule_detail,rule_matchs,filter,source_ip,source_http_port,source_https_port,rule_order_time}
      local create_result,create_err = db_query.query_mysql(create_sql,create_sql_params)
       if not create_result then
          response.fail_response(create_err)
      end
...
local function load_custom_upstream_address_data(user_name,group_name,custom_upstream_address_data)
    for k,v in pairs(custom_upstream_address_data) do
        local rule_name = v['rule_name']
        local rule_detail = v['rule_detail']
        local rule_matchs = v['rule_matchs']
        local filter = v['filter']
        local source_ip = v['source_ip']
        local source_http_port = v['source_http_port']
        local source_https_port = v['source_https_port']
        local status = v['status']
        local rule_order_time = v['rule_order_time']
        local sql = "DELETE FROM jxwaf_waf_group_custom_upstream_address WHERE user_name = ? AND group_name = ? AND rule_name = ? ;"
        local sql_params = {user_name,group_name,rule_name}
        local sql_result,sql_error = db_query.query_mysql(sql,sql_params)
        if not sql_result then
          response.fail_response(sql_error)
        end
        local create_sql = "INSERT INTO jxwaf_waf_group_custom_upstream_address (user_name,group_name, rule_name, rule_detail,rule_matchs,filter,source_ip,source_http_port,source_https_port,rule_order_time,status) VALUES (?,?,?,?,?,?,?,?,?,?,?);"
        local create_sql_params = {user_name,group_name,rule_name,rule_detail,rule_matchs,filter,source_ip,source_http_port,source_https_port,rule_order_time,status}
        local create_result,create_err = db_query.query_mysql(create_sql,create_sql_params)
        if not create_result then
          response.fail_response(create_err)
        end
...
function _M.load_group_custom_upstream_address_hub_config()
    local user_name = login_check.get_session()
    ...
    local custom_upstream_address_data = res_body['custom_upstream_address_data']
    if not custom_upstream_address_data then
        if auth_code and #auth_code ~= 0 then
            response.fail_response("auth_code is error")
        else
            response.fail_response("custom_upstream_address_data is nil")
        end
    end
    if force_load == "false" then
        for k, v in pairs(custom_upstream_address_data) do
            local sql = "SELECT COUNT(*) as count FROM jxwaf_waf_group_custom_upstream_address WHERE user_name = ? AND group_name = ?  AND rule_name = ?;"
    ...
    load_custom_upstream_address_data(user_name,group_name,custom_upstream_address_data)
    response.success_response("load success")
end

function _M.export_group_custom_upstream_address_hub_config()
  local user_name = login_check.get_session()
  local check_param = {"custom_upstream_address","group_name"}
  local body_data = request_data.get_body_data(check_param)
  local custom_upstream_address = body_data['custom_upstream_address']
  local group_name = body_data['group_name']
  local custom_upstream_address_data = {}
  for _,rule_name in ipairs(custom_upstream_address) do
      local sql = "SELECT * FROM jxwaf_waf_group_custom_upstream_address  WHERE `user_name` = ? AND `group_name` = ? AND `rule_name` = ?;"
      local sql_params = {user_name,group_name,rule_name}
      ...
      custom_upstream_address_data[rule_name] = {
        rule_name = result[1]['rule_name'],
        rule_detail = result[1]['rule_detail'],
        rule_matchs = result[1]['rule_matchs'],
        filter = result[1]['filter'],
        source_ip = result[1]['source_ip'],
        source_http_port = result[1]['source_http_port'],
        source_https_port = result[1]['source_https_port'],
    ...
    cjson.encode_empty_table_as_object(false)
    local response_message = {
        custom_upstream_address_data = custom_upstream_address_data,
        result = true
    }
    response.raw_success_response(response_message)
end
