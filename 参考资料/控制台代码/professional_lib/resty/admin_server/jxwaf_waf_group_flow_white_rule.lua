...
function _M.get_group_flow_white_rule_list()
...
local count_sql = "SELECT COUNT(*) AS total FROM jxwaf_waf_group_flow_white_rule WHERE `user_name` = ? AND `group_name` = ? ;"
  ...
  local sql = "SELECT * FROM jxwaf_waf_group_flow_white_rule  WHERE `user_name` = ? AND `group_name` = ? ORDER BY rule_order_time ASC LIMIT ? OFFSET ?;"
...
function _M.get_group_flow_white_rule()
  local user_name = login_check.get_session()
  local check_param = {"group_name","rule_name"}
  local body_data = request_data.get_body_data(check_param)
  local group_name = body_data['group_name']
  local rule_name = body_data['rule_name']
  local sql = "SELECT * FROM jxwaf_waf_group_flow_white_rule  WHERE `user_name` = ? AND `group_name` = ? AND `rule_name` = ?;"
...
function _M.create_group_flow_white_rule()
  local user_name = login_check.get_session()
  ...
  local count_sql = "SELECT COUNT(*) as count FROM jxwaf_waf_group_flow_white_rule WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
  ...
  local create_sql = "INSERT INTO jxwaf_waf_group_flow_white_rule (user_name,group_name, rule_name, rule_detail,rule_matchs,rule_action,action_value,rule_order_time) VALUES (?,?,?,?,?,?,?,?);"
...
function _M.delete_group_flow_white_rule()
  local user_name = login_check.get_session()
  local check_param = {"group_name","rule_name"}
  local body_data = request_data.get_body_data(check_param)
  local group_name = body_data['group_name']
  local rule_name = body_data['rule_name']
  local sql = "DELETE FROM jxwaf_waf_group_flow_white_rule WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
...
function _M.edit_group_flow_white_rule()
  local user_name = login_check.get_session()
  local check_param = {"group_name","rule_name","rule_detail","rule_matchs","rule_action","action_value"}
  local body_data = request_data.get_body_data(check_param)
  local group_name = body_data['group_name']
  ...
  local rule_action = body_data['rule_action']
  local action_value = body_data['action_value']
  local sql = "UPDATE jxwaf_waf_group_flow_white_rule  SET  rule_detail = ?,rule_matchs = ?,rule_action = ? ,action_value = ? WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
...
function _M.edit_group_flow_white_rule_status()
  local user_name = login_check.get_session()
  ...
  local sql = "UPDATE jxwaf_waf_group_flow_white_rule  SET  status = ? WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
...
function _M.exchange_group_flow_white_rule_priority()
  local user_name = login_check.get_session()
  ...
  if exchange_type == "top" then
    local sql = "SELECT rule_order_time FROM jxwaf_waf_group_flow_white_rule  WHERE `user_name` = ? AND `group_name` = ? ORDER BY `rule_order_time` ASC limit 1;"
    ...
    local exchange_sql = "UPDATE jxwaf_waf_group_flow_white_rule  SET  rule_order_time = ? WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
  ...
  elseif exchange_type == "exchange" then
    ...
    local rule_query_sql = "SELECT rule_order_time FROM jxwaf_waf_group_flow_white_rule  WHERE `user_name` = ? AND `group_name` = ? AND `rule_name` = ?;"
    ...
    local exchange_query_sql = "SELECT rule_order_time FROM jxwaf_waf_group_flow_white_rule  WHERE `user_name` = ? AND `group_name` = ? AND `rule_name` = ?;"
    ...
    local rule_update_sql = "UPDATE jxwaf_waf_group_flow_white_rule  SET  rule_order_time = ? WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
    ...
    local exchange_update_sql = "UPDATE jxwaf_waf_group_flow_white_rule  SET  rule_order_time = ? WHERE group_name = ? AND user_name = ? AND rule_name = ?;"
...
function _M.backup_group_flow_white_rule()
  local user_name = login_check.get_session()
  ...
  ngx.status = 200
  ngx.header.content_type = 'application/json'
  ngx.header['Content-Disposition'] = 'attachment; filename="flow_white_rule_data.json"'
  ngx.say(cjson.encode(rules))
  return ngx.exit(200)
end

function _M.load_group_flow_white_rule()
  local user_name = login_check.get_session()
...
local function load_flow_white_rule_data(user_name,group_name,flow_white_rule_data)
    for k,v in pairs(flow_white_rule_data) do
        ...
        local sql = "DELETE FROM jxwaf_waf_group_flow_white_rule WHERE user_name = ? AND group_name = ? AND rule_name = ? ;"
        local sql_params = {user_name,group_name,rule_name}
        local sql_result,sql_error = db_query.query_mysql(sql,sql_params)
        if not sql_result then
          response.fail_response(sql_error)
        end
        local create_sql = "INSERT INTO jxwaf_waf_group_flow_white_rule (user_name,group_name, rule_name, rule_detail,rule_matchs,rule_action,action_value,rule_order_time,status) VALUES (?,?,?,?,?,?,?,?,?);"
...
function _M.load_group_flow_white_rule_hub_config()
    local user_name = login_check.get_session()
    ...
    local flow_white_rule_data = res_body['flow_white_rule_data']
    if not flow_white_rule_data then
        if auth_code and #auth_code ~= 0 then
            response.fail_response("auth_code is error")
        else
            response.fail_response("flow_white_rule_data is nil")
        end
    end
    if force_load == "false" then
        for k, v in pairs(flow_white_rule_data) do
            local sql = "SELECT COUNT(*) as count FROM jxwaf_waf_group_flow_white_rule WHERE user_name = ? AND group_name = ?  AND rule_name = ?;"
    ...
    load_flow_white_rule_data(user_name,group_name,flow_white_rule_data)
    response.success_response("load success")
end

function _M.export_group_flow_white_rule_hub_config()
  local user_name = login_check.get_session()
  local check_param = {"flow_white_rule","group_name"}
  local body_data = request_data.get_body_data(check_param)
  local flow_white_rule = body_data['flow_white_rule']
  local group_name = body_data['group_name']
  local flow_white_rule_data = {}
  for _,rule_name in ipairs(flow_white_rule) do
      local sql = "SELECT * FROM jxwaf_waf_group_flow_white_rule  WHERE `user_name` = ? AND `group_name` = ? AND `rule_name` = ?;"
      local sql_params = {user_name,group_name,rule_name}
      ...
      flow_white_rule_data[rule_name] = {
    ...
    cjson.encode_empty_table_as_object(false)
    local response_message = {
        flow_white_rule_data = flow_white_rule_data,
        result = true
    }
    response.raw_success_response(response_message)
end
