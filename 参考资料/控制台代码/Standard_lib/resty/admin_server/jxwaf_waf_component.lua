...
function _M.get_component_list()
  ...
  local count_sql = "SELECT COUNT(*) AS total FROM jxwaf_waf_component WHERE `user_name` = ? ;"
  ...
  local sql = "SELECT * FROM jxwaf_waf_component  WHERE `user_name` = ?  ORDER BY rule_order_time ASC LIMIT ? OFFSET ?;"
...
function _M.get_component()
  local user_name = login_check.get_session()
  local check_param = {"name"}
  local body_data = request_data.get_body_data(check_param)
  local name = body_data['name']
  local sql = "SELECT * FROM jxwaf_waf_component  WHERE `user_name` = ? AND `name` = ? ;"
  local sql_params = {user_name,name}
  ...
  if query_result and #query_result > 0 then
    ...
    response.fail_response("name is not exist")
  end
end

function _M.create_component()
  local user_name = login_check.get_session()
  local check_param = {"name","detail","code","conf"}
  local body_data = request_data.get_body_data(check_param)
  local name = body_data['name']
  local detail = body_data['detail']
  local code = body_data['code']
  local conf = body_data['conf']
  local count_sql = "SELECT COUNT(*) as count FROM jxwaf_waf_component WHERE name = ? AND user_name = ? ;"
  local count_sql_params = {name,user_name}
  ...
  if tonumber(count_sql_result[1].count) > 0 then
    response.fail_response("name is exist")
  end
  local rule_order_time = math.floor(ngx.now())
  local create_sql = "INSERT INTO jxwaf_waf_component (user_name,name, detail,code, conf,rule_order_time) VALUES (?,?,?,?,?,?);"
  local create_sql_params = {user_name,name,detail,code,conf,rule_order_time}
...
function _M.delete_component()
  local user_name = login_check.get_session()
  local check_param = {"name"}
  local body_data = request_data.get_body_data(check_param)
  local name = body_data['name']
  local sql = "DELETE FROM jxwaf_waf_component WHERE name = ? AND user_name = ? ;"
  local sql_params = {name,user_name}
...
function _M.edit_component()
  ...
  local code = body_data['code']
  local conf = body_data['conf']
  local sql = "UPDATE jxwaf_waf_component  SET  detail = ?,code = ?,conf = ?  WHERE name = ? AND user_name = ? ;"
  local sql_params = {detail,code,conf,name,user_name}
  local sql_result,sql_error = db_query.query_mysql(sql,sql_params)
  if not sql_result then
    response.fail_response(sql_error)
  end
...
function _M.edit_component_status()
  local user_name = login_check.get_session()
  local check_param = {"name","status"}
  local body_data = request_data.get_body_data(check_param)
  local name = body_data['name']
  local status = body_data['status']
  local sql = "UPDATE jxwaf_waf_component  SET  status = ? WHERE name = ? AND user_name = ?;"
  local sql_params = {status,name,user_name}
  local sql_result,sql_error = db_query.query_mysql(sql,sql_params)
  if not sql_result then
    response.fail_response(sql_error)
  end
...
function _M.exchange_component_priority()
  local user_name = login_check.get_session()
  local check_param = {"name","type"}
  local body_data = request_data.get_body_data(check_param)
  local name = body_data['name']
  local exchange_type = body_data['type']
  if exchange_type == "top" then
    local sql = "SELECT MIN(rule_order_time) as rule_order_time FROM jxwaf_waf_component WHERE `user_name` = ? ;"
    ...
    local exchange_sql = "UPDATE jxwaf_waf_component  SET  rule_order_time = ? WHERE name = ? AND user_name = ? ;"
    local exchange_sql_params = {rule_order_time,name,user_name}
  ...
  elseif exchange_type == "exchange" then
    local exchange_name = param_check(body_data['exchange_name'])
    local rule_query_sql = "SELECT rule_order_time FROM jxwaf_waf_component  WHERE `user_name` = ? AND `name` = ? ;"
    local rule_query_sql_params = {user_name,name}
    ...
    local exchange_query_sql = "SELECT rule_order_time FROM jxwaf_waf_component  WHERE `user_name` = ? AND `name` = ?;"
    local exchange_query_sql_params = {user_name,exchange_name}
    ...
    local rule_update_sql = "UPDATE jxwaf_waf_component  SET  rule_order_time = ? WHERE name = ? AND user_name = ?  ;"
    local rule_update_sql_params = {exchange_query_rule_order_time,name,user_name}
    ...
    local exchange_update_sql = "UPDATE jxwaf_waf_component  SET  rule_order_time = ? WHERE name = ? AND user_name = ? ;"
    local exchange_update_sql_params = {rule_query_rule_order_time,exchange_name,user_name}
...
function _M.backup_component()
  local user_name = login_check.get_session()
  local check_param = {"name_list"}
  local body_data = request_data.get_body_data(check_param)
  local name_list = body_data['name_list']
  local rules = {}
  for _,name in ipairs(name_list) do
    ...
    local rule_conf = {
      name = query_result[1]['name'],
      detail = query_result[1]['detail'],
      code = query_result[1]['code'],
      conf = query_result[1]['conf']
    }
    table.insert(rules,rule_conf)
  end
  ngx.status = 200
  ngx.header.content_type = 'application/json'
  ngx.header['Content-Disposition'] = 'attachment; filename="component_data.json"'
  ngx.say(cjson.encode(rules))
  return ngx.exit(200)
end

function _M.load_component()
  local user_name = login_check.get_session()
  ...
  for _,rule in ipairs(rules) do
    ...
    local detail = rule['detail']
    local code = rule['code']
    local conf = rule['conf']
    local count_sql = "SELECT COUNT(*) as count FROM jxwaf_waf_component WHERE name = ? AND user_name = ?;"
    local count_sql_params = {name,user_name}
    local count_sql_result,count_error = db_query.query_mysql(count_sql,count_sql_params)
    if not count_sql_result then
...
local function load_component_data(user_name,component_data)
    for k,v in pairs(component_data) do
        local name = v['name']
        ...
        local create_sql = "INSERT INTO jxwaf_waf_component (user_name,name, detail,code, conf,rule_order_time,status) VALUES (?,?,?,?,?,?,?);"
        local create_sql_params = {user_name,name,detail,code,conf,rule_order_time,status}
        local create_result,create_err = db_query.query_mysql(create_sql,create_sql_params)
        if not create_result then
          response.fail_response(create_err)
        end
...
function _M.load_component_hub_config()
    local user_name = login_check.get_session()
    ...
    local component_data = res_body['component_data']
    if not component_data then
        if auth_code and #auth_code ~= 0 then
            response.fail_response("auth_code is error")
        else
            response.fail_response("component_data is nil")
        end
    end
    if force_load == "false" then
        for k, v in pairs(component_data) do
            local sql = "SELECT COUNT(*) as count FROM jxwaf_waf_component WHERE name = ? AND user_name = ? ;"
            local count_sql_result,count_sql_error = db_query.query_mysql(sql,{k,user_name})
            if not count_sql_result then
              response.fail_response(count_sql_error)
            end
            if tonumber(count_sql_result[1].count) > 0  then
              response.fail_response("name is exist "..k)
            end
        end
    end
    load_component_data(user_name,component_data)
    response.success_response("load success")
end

function _M.export_component_hub_config()
  local user_name = login_check.get_session()
  local check_param = {"component"}
  local body_data = request_data.get_body_data(check_param)
  local component = body_data['component']
  ...
  for _,name in ipairs(component) do
      ...
      if not result or #result == 0 then
          response.fail_response("name is not exist: "..name)
      end
      component_data[name] = {
        name = result[1]['name'],
        detail = result[1]['detail'],
        code = result[1]['code'],
        conf = result[1]['conf'],
