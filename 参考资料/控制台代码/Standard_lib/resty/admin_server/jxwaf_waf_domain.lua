...
function _M.get_domain_list()
  ...
  local count_sql = "SELECT COUNT(*) AS total FROM jxwaf_waf_domain WHERE `user_name` = ? ;"
  ...
  local sql = "SELECT * FROM jxwaf_waf_domain  WHERE `user_name` = ?  LIMIT ? OFFSET ?;"
...
function _M.api_get_domain_list()
  local user_name = login_check.get_session()
  local sql = "SELECT * FROM jxwaf_waf_domain  WHERE `user_name` = ? ;"
...
function _M.get_domain_search_list()
  local user_name = login_check.get_session()
  local check_param = {"page","search_domain"}
  local body_data = request_data.get_body_data(check_param)
  local page = tonumber(body_data['page']) or 1
  local search_domain = body_data['search_domain']
  local pageSize = 50
  local offset = (page - 1) * pageSize
  local count_sql = "SELECT COUNT(*) AS total FROM jxwaf_waf_domain WHERE `user_name` = ?  AND `domain` LIKE CONCAT('%', ?, '%');"
  local count_sql_params = {user_name,search_domain}
  ...
  local total = count_result[1]["total"]
  local total_pages = math.ceil(total / pageSize)
  local sql = "SELECT * FROM jxwaf_waf_domain  WHERE `user_name` = ?  AND `domain` LIKE CONCAT('%', ?, '%') LIMIT ? OFFSET ?;"
  local sql_params = {user_name,search_domain,pageSize,offset}
  local query_result,query_error = db_query.query_mysql(sql,sql_params)
  if not query_result or query_error then
    response.fail_response(query_error)
  end
...
function _M.get_domain()
  local user_name = login_check.get_session()
  local check_param = {"domain"}
  local body_data = request_data.get_body_data(check_param)
  local domain = body_data['domain']
  local sql = "SELECT * FROM jxwaf_waf_domain  WHERE `user_name` = ?  AND `domain` = ?;"
  local sql_params = {user_name, domain}
  ...
  if query_result and #query_result > 0 then
    ...
    response.fail_response("domain is not exist")
  end
end
...
function _M.create_domain()
  local user_name = login_check.get_session()
  local check_param = {"domain","http","https","ssl_domain","source_ip","source_http_port","source_https_port","origin_protocol","balance_type","pre_proxy","real_ip_conf","connect_timeout","send_timeout","read_timeout","detail"}
  local body_data = request_data.get_body_data(check_param)
  local domain = body_data['domain']
  ...
  local create_sql = "INSERT INTO jxwaf_waf_domain (user_name, domain, http,https,ssl_domain,source_ip,source_http_port,source_https_port,origin_protocol,balance_type,pre_proxy,real_ip_conf,connect_timeout,send_timeout,read_timeout,waf_update_source_ip,detail) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?);"
  local create_sql_params = {user_name,domain,http,https,ssl_domain,source_ip,source_http_port,source_https_port,origin_protocol,balance_type,pre_proxy,real_ip_conf,connect_timeout,send_timeout,read_timeout,waf_update_source_ip,detail}
...
function _M.delete_domain()
  ...
  local body_data = request_data.get_body_data(check_param)
  local domain = body_data['domain']
  local sql = "DELETE FROM jxwaf_waf_domain WHERE user_name = ? AND domain = ?;"
  local sql_params = {user_name,domain}
...
function _M.edit_domain()
  ...
  local sql = "UPDATE jxwaf_waf_domain  SET  http = ?,https = ?,ssl_domain = ?,source_ip = ?,source_http_port = ?,source_https_port = ?,origin_protocol = ?,balance_type = ?,pre_proxy = ?,real_ip_conf = ?,connect_timeout = ?,send_timeout = ?,read_timeout = ?,waf_update_source_ip = ?,detail = ?  WHERE  user_name = ? AND domain = ?;"
  local sql_params = {http,https,ssl_domain,source_ip,source_http_port,source_https_port,origin_protocol,balance_type,pre_proxy,real_ip_conf,connect_timeout,send_timeout,read_timeout,waf_update_source_ip,detail,user_name,domain}
  local sql_result,sql_error = db_query.query_mysql(sql,sql_params)
  if not sql_result then
    response.fail_response(sql_error)
  end
