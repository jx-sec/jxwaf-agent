...
-- 【修复1】移除了 if not self.connected 判断，防止重试时 sock 为 nil 崩溃
-- 无论之前状态如何，都创建新的 socket 对象，因为 sock 是局部变量
...
-- 【修复2】发送彻底失败后，必须清空缓冲区，否则 buffer_size 永远不会减小
-- 导致后续所有日志因超过 drop_limit 被永久丢弃
...
self.sni_host = v
-- 新增 use_pool 配置处理
elseif k == "use_pool" then
if type(v) ~= "boolean" then
return nil, '"use_pool" must be a boolean'
end
self.use_pool = v
end
end
