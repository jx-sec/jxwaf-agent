...
local req_host = ngx.ctx.req_host or {}

local sys_conf_data  = waf.get_sys_conf_data()
local ctx_waf_log = ngx.ctx.waf_log

local waf_log = {}
waf_log['group_name'] = req_host['group_name']  or ""
...
if tonumber(upstream_response_time) then
    waf_log['upstream_response_time'] = upstream_response_time
else
    waf_log['upstream_response_time'] =  ""
end
...
local raw_resp_header_data = cjson.encode(raw_resp_headers)
if #raw_resp_header_data > 8192 then
  waf_log['raw_resp_headers'] = string_sub(raw_resp_header_data,1,8192)
else
...
if ngx.var.scheme == "https" then
    ...
    --jxwaf_ssl_fingerprint['ssl_session_ticket_hash'] = ngx.ctx.ssl_session_ticket_hash  or ""
...
local remote_log = true
local disable_remote_log = ngx.ctx.disable_remote_log
if type(disable_remote_log) == 'table' and ctx_waf_log then
    local waf_module = ctx_waf_log['waf_module']
    if type(disable_remote_log['waf_module']) == 'table' and disable_remote_log['waf_module'][waf_module]  then
        remote_log = false
    end

    local waf_policy = ctx_waf_log['waf_policy']
    if type(disable_remote_log['waf_policy']) == 'table' and disable_remote_log['waf_policy'][waf_policy]  then
        remote_log = false
    end

    local waf_action = ctx_waf_log['waf_action']
    if type(disable_remote_log['waf_action']) == 'table' and disable_remote_log['waf_action'][waf_action]  then
        remote_log = false
    end
end

if sys_conf_data["log_conf_remote"] == "true" and (ctx_waf_log or sys_conf_data["log_all"] == "true" ) and remote_log then
...
if sys_conf_data['webtds_check'] == "true" then
    local wtds_logger = logger_socket:new()
    local webtds_node_ip = sys_conf_data['webtds_node_ip']
    local webtds_node_port = sys_conf_data['webtds_node_port']
    local ok,err = wtds_logger:init{
        host = webtds_node_ip,
        port = tonumber(webtds_node_port),
        sock_type = "tcp",
        flush_limit = 1,
        timeout = 3000,
        max_retry_times = 1,
    ...
    if not ok then
        ngx.log(ngx.ERR,"webtds failed to initialize the logger: ",err)
        return
    end
    local _, send_err = wtds_logger:log(cjson.encode(waf_log).."\n")
    if send_err then
      ngx.log(ngx.ERR, "webtds failed to log message: ", send_err)
    end
end