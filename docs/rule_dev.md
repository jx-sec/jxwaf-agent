...
> 字段与枚举已对齐控制台与节点引擎行为（三版本引擎一致）。**控制台服务端对 rule_action 不做枚举校验（任意值透传入库），错误动作值会被静默入库但节点不执行**——`generate` 按本文档枚举做客户端校验，以 CLI 报错为准。
...
1. **观察优先**：测试环境验证拦截类规则直接用 `block`（无真实业务，无需观察）；生产环境首发可先 `watch`，SOC 日志观察无误报后再改 `block`
