---
name: profiles
description: 已实施安全防护方案 - Log4j/CC攻击/CDN源IP提取/API参数校验/IP黑名单/接口限速等案例
---
...
### 实现要点
- 使用 `require "resty.jxwaf.iputils"` 的 `ip_in_cidrs` 函数
- 仅当 CDN 网段匹配成功时才覆盖 `ngx.ctx.src_ip`
- 不匹配时保持原 src_ip 不变
...
### 名单：block_malicious_ip
- name_list_rule：`[{"key": "http_args", "value": "src_ip"}]`
- name_list_action：block
- name_list_expire："true"（临时）
- name_list_expire_time："3600"（1小时）

### 条目管理
通过外部 API 动态添加/删除 IP：
```
POST /api_create_global_name_list_item
{"name_list_name": "block_malicious_ip", "name_list_item": "1.2.3.4", "waf_auth": "..."}
```
...
### 规则：limit_api_rate
- filter："true"（启用匹配条件）
- matchs：`[{"match_args": [{"key": "http_args", "value": "path"}], "args_prepocess": ["none"], "match_operator": "str_prefix", "match_value": "/api/"}]`
- entity：`[{"key": "http_args", "value": "src_ip"}, {"key": "http_args", "value": "path"}]`
- stat_time：60
- exceed_count：100（峰值 QPS × 2）
- block_time：300
- rule_action：bot_check
- action_value：auto

### 要点
- entity 用 src_ip + path 双维度，避免不同接口互相影响
- exceed_count 严格大于才触发（100 表示第 101 次请求触发）
- bot_check(auto) 对正常用户无感，仅拦截自动化工具
