...
3. 观察优先：测试环境验证拦截类规则直接用 block（测试环境无真实业务，无需 watch 观察）；生产环境首发可先 watch 观察误报再转 block；放行类操作无需观察；紧急防护经用户确认后可直接 block。
...
# 官方测试环境（waf-demo，专业版）已配置在项目 config.json 的 test 环境，开箱即用
# config.json 缺失/为空时命令报错要求先配置；自建或重新配置用 test init（--base-url/--waf-auth/--test-url 必填，域名组留空自动发现）
...
测试环境（test 命令组）与自有环境命令严格隔离：test 命令固定只操作测试环境（忽略 `--env`），通用命令默认只操作 active 环境。配置文件为**项目目录下的 `config.json`**（与 jxwaf-cli 同级，含凭据严禁提交）；CLI 代码**不写死任何环境参数**——config.json 缺失或为空时命令直接报错并引导配置。**官方测试环境（waf-demo，专业版）已配置好**（`config.json` 的 `test` 环境），开箱即用；自有环境经 `config set` 配置。
