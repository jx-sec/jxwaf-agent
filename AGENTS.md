...
**文档时效**：本地 `docs/` 是快照；镜像版本、部署命令、字段规范等以 **docs.jxwaf.com** 官方最新文档为准（按版本分站 `/jxwaf-standard/`、`/jxwaf-professional/`、`/jxwaf-cloud/`；部署看各站 Deployment-Tutorial）。遇到与本地文档不一致或拿不准的字段，先查官方文档再操作。
...
# 官方沙盒初始化（独立命令组，与自有环境隔离）；沙盒凭据经环境变量注入（向官方获取）
...
jxwaf-cli --env <环境名> <命令>      # 单次指定环境（默认取配置中的 active；sandbox 命令组忽略此参数）
...
官方沙盒（sandbox 命令组）与自有环境命令严格隔离：sandbox 命令固定只操作沙盒环境（忽略 `--env`），通用命令默认只操作 active 环境。配置文件为**项目目录下的 `config.json`**（与 jxwaf-cli 同级，含凭据严禁提交）；沙盒测试站点地址（配置下发后访问验证，固定 `https://waf-demo.jxwaf.com:4443`）在沙盒环境定义的 `test_url` 字段，`sandbox init` 自动写入；沙盒凭据仅经 `JXWAF_OFFICIAL_MASTER_AUTH` 环境变量注入。

## 标准工作流

**环境就绪检查** → 需求分析 → 信息澄清 → 查 docs/ 文档 → generate 生成 → **官方沙盒 `sandbox verify` 一键闭环验证**（sandbox 命令组与自有环境隔离）→ 用户确认 → 生产 dry-run → --apply。

- **环境就绪检查（第一步，必须）**：先跑 `jxwaf-cli config show` 确认 `config.json` 已初始化（active 环境或沙盒环境存在）。未初始化时**提醒用户先初始化（沙盒 `sandbox init` / 自有环境 `config set`）并终止流程**，不进入 generate/apply/verify 环节；已初始化用 `config validate` 确认连通性。自有环境报 "admin api is not enabled" / "ip not allowed" 时，提醒用户按 README 环境前置条件修改控制台 docker-compose（`ADMIN_API_ENABLE: "true"` + `ADMIN_API_WHITELIST: "*"`，生产环境可收紧为固定 IP）并重启容器
...
- **文档时效**：本地 `docs/` 为快照；最新镜像版本、部署命令、字段规范以 **docs.jxwaf.com** 官方文档为准（按版本分站，部署看各站 Deployment-Tutorial），不一致时先查官方再操作
